...
# Defer importing the heavy simulator until the user runs the simulation to
# avoid blocking the Streamlit UI on module import (prevents black/blank page).
...
st.info("Status: app loaded. If you see a black screen, try reloading the page or check the Streamlit server logs in the terminal.")
...
with st.sidebar:
    ...
    enable_signals = st.checkbox("Enable signal planning", value=False)
    ...
    roundabout_factor = st.slider("Roundabout factor", min_value=0.5, max_value=1.0, value=0.9, step=0.05)

    # The button triggers computation; previously this also required the
    # checkbox to be enabled, which confused users. We'll compute whenever
    # the button is clicked. The checkbox is left for future toggling.
    compute_signal = st.button("Compute signal plan")
...
if compute_signal:
    ...
    phases = arm_labels
    ctrl = ModifiedWebsterRoundabout(phases=phases, saturation_flow=sat_flow,
                                     lost_time_per_phase=lost_time, roundabout_factor=roundabout_factor)
    cycle, greens = ctrl.get_signal_plan(flows_vph)

    st.subheader("Signal plan (Modified Webster)")
    st.metric("Cycle length (s)", f"{cycle:.1f}")

    g_df = pd.DataFrame.from_dict(greens, orient="index", columns=["green_s"])
    g_df["flow_vph"] = pd.Series(flows_vph)
    st.table(g_df)
...
# Defer importing the heavy simulator until the user runs the simulation to
# avoid blocking the Streamlit UI on module import (prevents black/blank page).
# The simulator classes will be imported inside `run_simulation()` below.
...
def normalize_turning(p: Tuple[float, float, float]) -> Tuple[float, float, float]:
    ...
    # If the user requested a signal plan, compute and show it in the main area
    if enable_signals and compute_signal:
        flows_vph = {
            "Arm0": a0 * 3600.0,
            "Arm1": a1 * 3600.0,
            "Arm2": a2 * 3600.0,
